...
此外，还有一些特殊功能，如自组织设计：整个系统对于用户是透明的，用户无需对网络层或应用层做任何配置，只需要将SoC接入系统，即可被系统识别并记录。
...
在代码实现中，该模块用于保存分配给设备的序列号，具体保存在地址`0x50-0x55`，这是自组织功能设计（6.1）的一部分。
